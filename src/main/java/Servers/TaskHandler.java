    ...
    private void getRequestProcessing(HttpExchange h, Optional<String> query, String path) throws IOException {
        ...
        switch (path) {
            case "/tasks/":
                ...
                break;
            case "/tasks/history/":
                jsonObject = gson.toJson(taskManager.history());
                sendJsonObject(h, jsonObject);
                break;
            case "/tasks/task/":
    ...
    private void pathTasksSubtaskEpicGetProcessing(HttpExchange h, Optional<String> query) throws IOException {
        if (!query.isEmpty()) {
            ...
            if (queryElements[0].equals("id") && isNumeric(queryElements[1])) {
                ...
                if (idIsExists && !taskManager.getSubtasksOfEpic(id).isEmpty()) {
                    jsonObject = gson.toJson(taskManager.getSubtasksOfEpic(id));
                    sendJsonObject(h, jsonObject);
                } else if (idIsExists && taskManager.getSubtasksOfEpic(id).isEmpty()) {
